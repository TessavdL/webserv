...
/*   Updated: 2022/11/15 16:12:42 by tevan-de      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
size_t	determine_location_index(std::string const& uri_path, std::vector<LocationBlock> const& v) {
	size_t		l = 0;
	size_t		max = 0;
	...
	for (size_t i = 0; i < v.size(); i++) {
		if (!v[i].get_path_and_optional_modifier()[0].empty()) {
			if (v[i].get_path_and_optional_modifier()[1] == uri_path) {
				return (i);
			}
			else {
				continue ;
		...
		l = longest_match(uri_path, v[i].get_path_and_optional_modifier()[1]);
		if (max < l) {
			max = l;
			location_index = i;
		}
